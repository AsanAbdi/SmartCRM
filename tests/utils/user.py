...
def authenticate_token_from_email(
    ...
    if not user:
        ...
        user = User.model_validate(user_in_create)
    else:
        if not user.id:
            raise Exception("User id not set")
        password_hash = get_password_hash(password)
        user.sqlmodel_update(udpate={"hashed_password": password_hash})
        db.add(user)
        db.commit()
        db.refresh(user)
    
    return user_authentication_headers(username=username, password=password)